...
_CALIB_POINTS = [
    ...
    (15000,  10,  7500,  420), (50000,  10,  7500,  560), (100000, 10,  7500,  650),
    # ─── D = 12 in ─────────────────────────────────────────────
    (15000,  12,  4500,  280), (50000,  12,  4500,  410),
    (100000, 12,  4500,  440), (200000, 12,  4500,  510),
    (15000,  12,  7500,  450), (50000,  12,  7500,  600), (100000, 12,  7500,  700),
    # ─── D = 14 in ─────────────────────────────────────────────
    (15000,  14,  4500,  300), (30000,  14,  4500,  350), (50000,  14,  4500,  420),
    (75000,  14,  4500,  450), (100000, 14,  4500,  470), (200000, 14,  4500,  600),
    (15000,  14,  3000,  250), (50000,  14,  3000,  350),
    (15000,  14,  6000,  400), (50000,  14,  6000,  550), (75000,  14,  6000,  600),
    (100000, 14,  6000,  630), (200000, 14,  6000,  800),
    (15000,  14,  7500,  460), (50000,  14,  7500,  600),
    (15000,  14, 15000,  760), (50000,  14, 15000, 1000),
    # ─── D = 16 in ─────────────────────────────────────────────
    (15000,  16,  4500,  300), (50000,  16,  4500,  440), (75000,  16,  4500,  470),
    (100000, 16,  4500,  500), (200000, 16,  4500,  600),
    (15000,  16,  6000,  400), (50000,  16,  6000,  550), (75000,  16,  6000,  600),
    (100000, 16,  6000,  690), (200000, 16,  6000,  850),
    (15000,  16,  7500,  460), (50000,  16,  7500,  650), (100000, 16,  7500,  850),
    # ─── D = 18 in ─────────────────────────────────────────────
    (15000,  18,  4500,  320), (50000,  18,  4500,  460), (75000,  18,  4500,  500),
    (100000, 18,  4500,  550), (200000, 18,  4500,  650),
    (15000,  18,  6000,  420), (50000,  18,  6000,  600), (75000,  18,  6000,  700),
    (100000, 18,  6000,  750), (200000, 18,  6000,  950),
    (15000,  18,  7500,  500), (50000,  18,  7500,  750), (100000, 18,  7500,  900),
    # ─── D = 20 in ─────────────────────────────────────────────
    (15000,  20,  4500,  330), (50000,  20,  4500,  490), (75000,  20,  4500,  530),
    (100000, 20,  4500,  600), (200000, 20,  4500,  750),
    (400000, 20,  4500,  990), (1000000,20,  4500, 1300),
    (15000,  20,  6000,  420), (50000,  20,  6000,  600), (75000,  20,  6000,  700),
    (100000, 20,  6000,  800), (200000, 20,  6000, 1000),
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.get('calc_project_name'):
            ...
            if st.button("🗑️ ล้างข้อมูลที่โหลด"):
                ...
                # ล้าง dynamic keys (layer + nomo อื่น) ที่ไม่มี widget key คงที่
                for lkey in list(st.session_state.keys()):
                    if lkey.startswith('calc_layer_') or                        lkey.startswith(('ls_select', 'k_corr', 'k_inf')):
    ...
    with tab1:
        ...
        with col2:
            # อ่านค่า k_eff จาก session_state ก่อน (widget จะอยู่ด้านล่าง)
            ...
            st.markdown("---")
            # ── k_eff input + Sensitivity Analysis ──────────────────────
            st.subheader("🎯 Effective k และ Optimum Analysis")
            ...
            if optimum_k is not None:
                ...
                if diff > 0:
                    ...
                    st.error(f"❌ Optimum k_eff = **{optimum_k:,} pci** | ใช้อยู่ต่ำกว่า {abs(diff):,} pci — ควรเพิ่ม k หรือ D")
            else:
                st.error("❌ ไม่พบ Optimum k_eff ในช่วง 50–1,000 pci")

            # ตาราง ±3 ชุดรอบ Optimum
            if optimum_k is not None:
            ...
            # เพิ่ม k_eff ที่ใช้อยู่ถ้าอยู่นอกช่วง
            k_eff_rounded = round(k_eff / 10) * 10
            if k_eff_rounded not in display_ks and 50 <= k_eff_rounded <= 1000:
                display_ks = sorted(set(display_ks) | {k_eff_rounded})
