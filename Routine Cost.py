...
def generate_word_report(include_gravel=True):
    ...
    add_thai_para(doc, "Ka = 1 + 0.50 × (X1 + X2 + X3 + X4 + X5 + X6 + Y1 + Y2 + Y3 + Y4 + Y5 + Y6)",
                  first_indent=False)
    add_table_word(doc,
        headers=["Factor", "คำอธิบาย", "ค่า (Lookup Table)"],
        rows=[
            ["X1", "ลักษณะผิวทางและพื้นทาง",          "High=0.00 / Intermediate=0.50 / Low=1.00"],
            ["X2", "CBR ดินเดิม (%)",                   "CBR≤2→1.00 / 2-3→0.75 / 3-4→0.50 / 4-5→0.25 / >5→0.00"],
            ["X3", "ปริมาณจราจร AADT / 2 ช่อง (คัน/วัน)", "0-500→0.00 … 5,701+→2.25 (ตารางคู่มือ)"],
            ["X4", "อายุบริการ (ปี)",                  "≤3→0.00 / 4→0.20 / 5→0.40 … 11→1.60 / ≥12→1.80"],
            ["X5", "ความกว้างผิวทาง / 2 ช่อง (ม.)",   "<5.50→0.00 / 5.50-5.99→0.02 / 6.00-6.49→0.05 / 6.50-6.99→0.10 / ≥7.00→0.19"],
            ["X6", "ภูมิประเทศ",                        "ที่ราบ(P)→0.00 / ลูกเนิน(R)→0.02 / ลูกเนินสลับเขา(RM)→0.04 / เขา(S)→0.07"],
            ["Y1", "ความกว้างเขตทาง (ม.)",              "≤40→0.00 / 40-60→0.10 / 60-80→0.20 / >80→0.30"],
            ["Y2", "ไหล่ทางกว้างสุด 1 ข้าง (ม.)",      "≤1.75→0.00 / 1.76-2.00→0.10 / 2.01-2.25→0.15 / >2.25→0.20"],
            ["Y3", "จราจรสงเคราะห์ (ภูมิประเทศ)",      "P→0.00 / R→0.24 / RM→0.36 / S→0.48"],
            ["Y4", "ท่อระบายน้ำ (ภูมิประเทศ)",          "P→0.00 / R→0.24 / RM→0.36 / S→0.48"],
            ["Y5", "สะพาน เฉลี่ย (ม./กม.)",            "≤21→0.00 / 21-25→0.02 / 25-30→0.04 / >30→0.06"],
            ["Y6", "ทำความสะอาดระบาย (ภูมิประเทศ)",     "P→0.00 / R→0.04 / RM→0.08 / S→0.12"],
        ],
        col_widths=[1.5, 5, 10],
    )
    doc.add_paragraph()
    ...
    add_table_word(doc,
        headers=["Factor", "คำอธิบาย", "ค่า (Lookup Table)"],
        rows=[
            ["Z1", "ดัชนีสภาพผิวทาง (1-8)",             "1→0.00 / 2→0.25 / 3→0.50 / 4→0.75 / 5→1.00 / 6→1.30 / 7→1.60 / 8→2.00"],
            ["Z2", "CBR ดินคันทาง (%)",                  "CBR≤2→1.00 / 2-3→0.75 / 3-4→0.50 / 4-5→0.25 / >5→0.00"],
            ["Z3", "ปริมาณจราจร AADT / 2 ช่อง (คัน/วัน)", "0-1,000→0.00 / 1,001-2,000→0.20 … 15,001+→3.00 (ตารางคู่มือ)"],
            ["Z4", "ความกว้างผิวทาง / 2 ช่อง (ม.)",      "<6.50→0.00 / 6.50-6.99→0.08 / ≥7.00→0.17"],
            ["Y1-Y6", "ปัจจัยเขตทาง / สิ่งก่อสร้าง",    "เดียวกับผิวแอสฟัลท์ (ดูตารางข้างต้น)"],
        ],
        col_widths=[1.5, 5, 10],
    )
    doc.add_paragraph()
