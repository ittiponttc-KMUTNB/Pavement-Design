...
def สร้างรายงาน_Word_ที่ปรึกษา(
    ...
    """
    สร้างรายงาน LCCA แบบที่ปรึกษา พร้อมบทเกริ่นนำ ข้อมูลโครงการ และบทสรุป
    หมายเลขหัวข้อหลักกำหนดได้ เช่น 4.8 → หัวข้อย่อยเริ่มที่ 4.8.1
    """
    if ข้อมูลโครงการ is None:
        ข้อมูลโครงการ = {}

    # แยกส่วนหมายเลขเพื่อสร้าง sub-heading
    def หัวข้อย่อย(n: int) -> str:
    ...
    # Helper: เพิ่ม heading พร้อมเบอร์หัวข้อ
    def add_h1(text):
    ...
    def add_para(text, bold=False):
        ...
        p.runs[0].font.name = 'TH Sarabun New' if p.runs else None
        if bold and p.runs:
            p.runs[0].bold = True
        return p
    ...
    # หัวข้อหลัก
    # ─────────────────────────────────────────────────────────
    add_h1(f"{หมายเลขหัวข้อหลัก}  การวิเคราะห์ต้นทุนตลอดอายุการใช้งานผิวทาง (LCCA)")

    # ─────────────────────────────────────────────────────────
    # 4.8.1 บทเกริ่นนำ
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(1)}  บทเกริ่นนำ")
    doc.add_paragraph(
        'การวิเคราะห์ต้นทุนตลอดอายุการใช้งาน (Life-Cycle Cost Analysis: LCCA) '
        'เป็นเครื่องมือทางเศรษฐศาสตร์วิศวกรรมที่ใช้ในการเปรียบเทียบทางเลือกการลงทุน '
        'โดยพิจารณาต้นทุนทั้งหมดตลอดอายุการใช้งานของโครงการ ประกอบด้วย '
        'ต้นทุนก่อสร้างเริ่มต้น ต้นทุนบำรุงรักษาประจำ ต้นทุนฟื้นฟูสภาพผิวทาง '
        'และมูลค่าซากปลายทาง โดยแปลงต้นทุนทั้งหมดมาเป็นมูลค่าปัจจุบัน (Present Worth: PW) '
        'เพื่อให้สามารถเปรียบเทียบได้บนฐานเวลาเดียวกัน'
    )
    doc.add_paragraph(
        'การศึกษานี้ดำเนินการวิเคราะห์ LCCA สำหรับโครงการ' + f' {ชื่อโครงการ} '
        'โดยพิจารณาทางเลือกผิวทางที่เป็นไปได้ทั้งหมด เพื่อคัดเลือกทางเลือกที่มีต้นทุน '
        'ตลอดอายุการใช้งานต่ำที่สุดและมีความคุ้มค่าทางเศรษฐศาสตร์สูงสุด '
        'ตามแนวทางของ FHWA Life-Cycle Cost Analysis และมาตรฐาน AASHTO'
    )
    ...
    # 4.8.2 ข้อมูลโครงการ
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(2)}  ข้อมูลโครงการ")

    # ตารางข้อมูลโครงการ
    table_proj = doc.add_table(rows=1, cols=2)
    table_proj.style = 'Table Grid'
    table_proj.alignment = WD_TABLE_ALIGNMENT.CENTER
    hdr = table_proj.rows[0].cells
    hdr[0].text = 'รายละเอียด'
    hdr[1].text = 'ข้อมูล'
    for cell in hdr:
    ...
    ฟิลด์ = [
        ...
        ('ผู้รับผิดชอบโครงการ', ข้อมูลโครงการ.get('ผู้รับผิดชอบ', '-')),
        ('วันที่วิเคราะห์', datetime.now().strftime("%d/%m/%Y")),
        ('ระยะเวลาวิเคราะห์', f'{ระยะวิเคราะห์} ปี'),
        ('อัตราคิดลด (Real Discount Rate)', f'{อัตราคิดลด*100:.1f}%'),
        ('จำนวนทางเลือกที่วิเคราะห์', f'{len(สรุป)} ทางเลือก'),
    ]
    for label, value in ฟิลด์:
        row = table_proj.add_row().cells
        row[0].text = label
    ...
    # ─────────────────────────────────────────────────────────
    # 4.8.3 ทางเลือกผิวทางที่วิเคราะห์
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(3)}  ทางเลือกผิวทางที่วิเคราะห์")
    ...
    for _, row in สรุป.iterrows():
        ...
        r[1].text = str(row['ประเภทผิวทาง'])
        r[2].text = f"{row['ความหนา_ซม']:.1f}"
        r[3].text = f"{row['พื้นที่_ตรม']:,.0f}"
        r[4].text = f"{row['ต้นทุนก่อสร้าง_ตรม']:,.0f}"

    doc.add_paragraph()

    # ─────────────────────────────────────────────────────────
    # 4.8.4 แผนบำรุงรักษาและฟื้นฟูสภาพ
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(4)}  แผนบำรุงรักษาและฟื้นฟูสภาพ")

    for ท in ทางเลือกทั้งหมด:
        if not ท.เปิดใช้งาน:
            continue
        p = doc.add_paragraph()
        run = p.add_run(f"ก) {ท.ชื่อ}")
        run.bold = True
        run.font.name = 'TH Sarabun New'

        # บำรุงรักษา
        doc.add_paragraph('  แผนบำรุงรักษา:')
        for ม in ท.แผนบำรุงรักษา:
            freq_text = f"ทุก {ม.ความถี่} ปี" if ม.ความถี่ > 0 else "ครั้งเดียว"
            doc.add_paragraph(f"    - {ม.ชื่อกิจกรรม}: {ม.ต้นทุนต่อหน่วย:,.0f} บาท/ตร.ม. เริ่มปีที่ {ม.ปีเริ่มต้น} ({freq_text})")

        # ฟื้นฟูสภาพ
        doc.add_paragraph('  แผนฟื้นฟูสภาพ:')
        for ฟ in ท.แผนฟื้นฟูสภาพ:
            doc.add_paragraph(f"    - {ฟ.ชื่อกิจกรรม}: {ฟ.ต้นทุนต่อหน่วย:,.0f} บาท/ตร.ม. ปีที่ {ฟ.ปีดำเนินการ}")

    doc.add_paragraph()

    # ─────────────────────────────────────────────────────────
    # 4.8.5 ผลการวิเคราะห์ LCCA
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(5)}  ผลการวิเคราะห์ LCCA")
    ...
    for _, row in สรุป.iterrows():
        ...
        r[0].text = str(int(row['ลำดับ']))
        r[1].text = str(row['ทางเลือก'])
        r[2].text = f"{row['มูลค่าปัจจุบันรวม']:,.0f}"
        r[3].text = f"{row['ต้นทุนเฉลี่ยรายปี']:,.0f}"
        r[4].text = f"{row['ต้นทุนต่อตรม_ต่อปี']:,.2f}"

    doc.add_paragraph()

    # ─────────────────────────────────────────────────────────
    # 4.8.6 องค์ประกอบต้นทุน
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(6)}  องค์ประกอบต้นทุน (มูลค่าปัจจุบัน)")
    ...
    # ─────────────────────────────────────────────────────────
    # 4.8.7 บทสรุปและข้อเสนอแนะ
    # ─────────────────────────────────────────────────────────
    add_h2(f"{หัวข้อย่อย(7)}  บทสรุปและข้อเสนอแนะ")
    ...
    บทสรุป = (
        ...
        f"พบว่า {ผู้ชนะ['ทางเลือก']} เป็นทางเลือกที่มีความคุ้มค่าทางเศรษฐศาสตร์สูงสุด "
        f"โดยมีมูลค่าปัจจุบันรวมเท่ากับ {ผู้ชนะ['มูลค่าปัจจุบันรวม']:,.0f} บาท "
        f"และต้นทุนเฉลี่ยรายปี (EAC) ที่ {ผู้ชนะ['ต้นทุนเฉลี่ยรายปี']:,.0f} บาท/ปี "
        f"ตลอดระยะเวลาวิเคราะห์ {ระยะวิเคราะห์} ปี ที่อัตราคิดลด {อัตราคิดลด*100:.1f}%"
    )
    doc.add_paragraph(บทสรุป)
    ...
    if len(สรุป) > 1:
        ...
        for idx in range(1, len(สรุป)):
            ...
            ส่วนต่าง = อื่น['มูลค่าปัจจุบันรวม'] - ผู้ชนะ['มูลค่าปัจจุบันรวม']
            ร้อยละ = (ส่วนต่าง / อื่น['มูลค่าปัจจุบันรวม']) * 100
            เปรียบ_parts.append(f"ประหยัดกว่า {อื่น['ทางเลือก']} คิดเป็นร้อยละ {ร้อยละ:.1f} ({ส่วนต่าง:,.0f} บาท)")
        doc.add_paragraph(
            f"เมื่อเปรียบเทียบกับทางเลือกอื่น {ผู้ชนะ['ทางเลือก']} "
            + ', '.join(เปรียบ_parts[:2]) +
            (f" และ{เปรียบ_parts[2]}" if len(เปรียบ_parts) > 2 else "")
        )

    doc.add_paragraph(
        f"ดังนั้น จึงมีข้อเสนอแนะให้เลือกใช้ {ผู้ชนะ['ทางเลือก']} เป็นทางเลือกหลักในการออกแบบ "
        "เนื่องจากให้ต้นทุนรวมต่ำที่สุดตลอดอายุการใช้งานของโครงการ "
        "สอดคล้องกับหลักการ Value for Money และแนวทางการจัดสรรงบประมาณโครงสร้างพื้นฐานที่มีประสิทธิภาพ"
    )

    doc.add_paragraph()

    # หมายเหตุ
    note = doc.add_paragraph('หมายเหตุ: การวิเคราะห์ LCCA ดำเนินการตามมาตรฐาน FHWA-SA-98-079 '
                              'และ AASHTO Guide for Design of Pavement Structures '
                              f'อัตราคิดลด {อัตราคิดลด*100:.1f}% เป็น Real Discount Rate (ไม่รวมอัตราเงินเฟ้อ)')
    ...
    doc.add_paragraph('─' * 50)
    doc.add_paragraph(f'โครงการ: {ชื่อโครงการ}')
    doc.add_paragraph('รายงานนี้สร้างโดย: โปรแกรมวิเคราะห์ LCCA ผิวทาง v2.2')
    doc.add_paragraph('ภาควิชาครุศาสตร์โยธา มหาวิทยาลัยเทคโนโลยีพระจอมเกล้าพระนครเหนือ')
...
def main():
    ...
    **พัฒนาโดย:** รศ.ดร.อิทธิพล มีผล (Assoc. Prof. Dr. Ittipon Meepon)  
    ...
    with st.sidebar:
        ...
        # ─── ข้อมูลโครงการสำหรับรายงานแบบที่ปรึกษา ───
        with st.expander("📋 ข้อมูลโครงการ (รายงานที่ปรึกษา)", expanded=False):
            if 'ข้อมูลโครงการ' not in st.session_state:
                st.session_state['ข้อมูลโครงการ'] = {}

            st.session_state['ข้อมูลโครงการ']['ชื่อโครงการ'] = ชื่อโครงการ

            สายทาง = st.text_input("สายทาง / เส้นทาง",
                value=st.session_state['ข้อมูลโครงการ'].get('สายทาง', ''),
                key="proj_route")
            st.session_state['ข้อมูลโครงการ']['สายทาง'] = สายทาง

            ตอน = st.text_input("ตอน",
                value=st.session_state['ข้อมูลโครงการ'].get('ตอน', ''),
                key="proj_section_name")
            st.session_state['ข้อมูลโครงการ']['ตอน'] = ตอน

            ที่ตั้ง = st.text_input("ที่ตั้งโครงการ (จังหวัด)",
                value=st.session_state['ข้อมูลโครงการ'].get('ที่ตั้ง', ''),
                key="proj_location")
            st.session_state['ข้อมูลโครงการ']['ที่ตั้ง'] = ที่ตั้ง

            ความยาว = st.text_input("ความยาวโครงการ (กม.)",
                value=st.session_state['ข้อมูลโครงการ'].get('ความยาว', ''),
                key="proj_length")
            st.session_state['ข้อมูลโครงการ']['ความยาว'] = ความยาว

            ผู้รับผิดชอบ = st.text_input("ผู้รับผิดชอบ / ผู้จัดทำ",
                value=st.session_state['ข้อมูลโครงการ'].get('ผู้รับผิดชอบ', ''),
                key="proj_owner")
            st.session_state['ข้อมูลโครงการ']['ผู้รับผิดชอบ'] = ผู้รับผิดชอบ

            st.divider()
            st.caption("📌 กำหนดหมายเลขหัวข้อรายงาน")
            หมายเลขหัวข้อ = st.text_input(
                "หมายเลขหัวข้อหลัก",
                value=st.session_state.get('หมายเลขหัวข้อ', '4.8'),
                key="section_number",
                help="เช่น 4.8 จะได้ 4.8.1, 4.8.2, 4.8.3 ..."
            )
            st.session_state['หมายเลขหัวข้อ'] = หมายเลขหัวข้อ
            st.caption(f"ตัวอย่าง: {หมายเลขหัวข้อ}.1 บทเกริ่นนำ | {หมายเลขหัวข้อ}.2 ข้อมูลโครงการ | {หมายเลขหัวข้อ}.7 บทสรุป")
    ...
    with tab2:
        ...
        if len(ทางเลือกที่ใช้) == 0:
            ...
            if DOCX_AVAILABLE:
                หมายเลข = st.session_state.get('หมายเลขหัวข้อ', '4.8')
                ข้อมูลโครงการ = st.session_state.get('ข้อมูลโครงการ', {})
                ...
                with col_pro1:
                    st.download_button(
                        label=f"📄 สร้างรายงานแบบที่ปรึกษา (หัวข้อ {หมายเลข})",
                        data=word_file_pro,
                ...
                with col_pro2:
                    st.info(f"หัวข้อ: {หมายเลข}  \nย่อย: {หมายเลข}.1 ~ {หมายเลข}.7")
            else:
                st.warning("⚠️ ต้องติดตั้ง python-docx: `pip install python-docx`")
