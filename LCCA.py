...
def อ่านข้อมูลจาก_excel(uploaded_file) -> Dict[str, Dict]:
    ...
    try:
        ...
        for idx, row in df.iterrows():
            ...
            # ข้ามแถวที่ว่างหรือเป็นตัวอย่าง
            if ชื่อย่อ in ['', 'nan', 'NaN'] or ชื่อย่อ not in ชื่อแมพ:
                continue
...
def main():
    ...
    st.markdown("""
    **Life-Cycle Cost Analysis for Pavement Alternatives - Version 2.1**
        ...
        if uploaded_json is not None:
            try:
                ...
                if 'ทางเลือก' not in data:
                    ...
                    st.success(f"✅ โหลดโครงการสำเร็จ!")
                    st.info(f"📅 บันทึกเมื่อ: {วันที่บันทึก}")
                    st.rerun()
        ...
        for i, ทางเลือก in enumerate(st.session_state.ทางเลือกทั้งหมด):
            with st.expander(f"{'✅' if ทางเลือก.เปิดใช้งาน else '❌'} ทางเลือกที่ {i+1}: {ทางเลือก.ชื่อ}", expanded=(i==0)):
                ...
                with col2:
                    ...
                    # ถ้ามีข้อมูล upload และมีประเภท
                    ...
                    ประเภทใหม่ = st.selectbox(
                        ...
                        index=["Flexible", "JPCP", "JRCP", "CRCP"].index(ประเภทเริ่มต้น) if ประเภทเริ่มต้น in ["Flexible", "JPCP", "JRCP", "CRCP"] else 0,
                ...
                with col3:
                    ...
                    if ทางเลือก.ชื่อ in uploaded_data and isinstance(uploaded_data[ทางเลือก.ชื่อ], dict):
                        uploaded_thickness = uploaded_data[ทางเลือก.ชื่อ].get('ความหนา')
                        if uploaded_thickness is not None:  # ✅ เช็คว่าไม่ใช่ None
                            ความหนาปัจจุบัน = uploaded_thickness
                    
                    # ถ้ายังเป็น 0 ให้ใช้ค่าเริ่มต้นตามประเภท
                ...
                with col4:
                    ...
                    # ถ้ามีข้อมูล upload ให้ใช้ข้อมูลนั้น (รองรับทั้ง dict เก่าและใหม่)
                    if ทางเลือก.ชื่อ in uploaded_data:
                        uploaded_info = uploaded_data[ทางเลือก.ชื่อ]
                        # ตรวจสอบว่าเป็น dict ใหม่ (มี ต้นทุน, ความหนา, ประเภท) หรือ float เก่า
                        if isinstance(uploaded_info, dict):
                            ต้นทุนเริ่มต้น = uploaded_info.get('ต้นทุน', ทางเลือก.ต้นทุนก่อสร้าง)
                        else:
                            ต้นทุนเริ่มต้น = uploaded_info  # backward compatible
                    else:
                        ต้นทุนเริ่มต้น = ทางเลือก.ต้นทุนก่อสร้าง
                ...
                for k, ฟื้นฟู in enumerate(ทางเลือก.แผนฟื้นฟูสภาพ):
                    ...
                    with col_r1:
                        ...
                        # ✅ เช็คและอัพเดทต้นทุนทันทีเมื่อชื่อเปลี่ยน
                        if ชื่อกิจกรรมใหม่ != ฟื้นฟู.ชื่อกิจกรรม:
                            ฟื้นฟู.ชื่อกิจกรรม = ชื่อกิจกรรมใหม่
                            # ถ้าชื่อมี "ก่อสร้างใหม่" → อัพเดทต้นทุน
                    ...
                    with col_r2:
                        ...
                        ฟื้นฟู.ต้นทุนต่อหน่วย = st.number_input(
                            ...
                            label_visibility="collapsed",
                            help="💡 กิจกรรม 'ก่อสร้างใหม่' จะใช้ต้นทุนก่อสร้างเริ่มต้น"
                        )
